...
/**
 * Panneau de configuration des équipes pour le mode Trio Local (3 joueurs).
 * 
 ...
 */
public class TrioConfigMenuPanel extends JPanel {

    private static final int NUM_TEAMS = 3;

    private int selectedTeam = 0; // 0, 1 ou 2
    private int selectedField = 0; // 0 = type, 1 = niveau IA ou contrôle, 2 = couleur, 3 = nombre

    // Configuration des équipes
    private String[] teamTypes = { "Humain", "Humain", "Humain" };
    private String[] aiLevels = { "Facile", "Facile", "Facile" }; // 3 niveaux disponibles
    private String[] controlTypes = { "Flèches", "Souris", "ZQSD" }; // Types de contrôle pour humains
    private int[] fighterCounts = { 2000, 2000, 2000 }; // Nombre de combattants par équipe
    private int[] selectedColorIndices = { 0, 1, 2 }; // Index des couleurs choisies (0-5)
    ...
    private Color[] currentTeamColors = {
            ...
            RenderConfig.TEAM_COLORS[2] // Vert par défaut
    };
    ...
    public interface ConfigCompleteListener {
        void onConfigComplete(TrioConfiguration config);
    }
    ...
    public static class TrioConfiguration {
        ...
        public TrioConfiguration(String[] types, String[] levels, String[] controls, int[] counts, int[] colors) {
    ...
    private void tryStartGame() {
        // Vérifier les conflits de contrôle entre joueurs humains
        java.util.List<Integer> humanTeams = new java.util.ArrayList<>();
        for (int i = 0; i < NUM_TEAMS; i++) {
            if ("Humain".equals(teamTypes[i])) {
                humanTeams.add(i);
            }
        }

        // Vérifier les doublons de contrôle parmi les humains
        java.util.Set<String> usedControls = new java.util.HashSet<>();
        for (int teamIndex : humanTeams) {
            String control = controlTypes[teamIndex];
        ...
        for (int i = 0; i < NUM_TEAMS; i++) {
            int colorIndex = selectedColorIndices[i];
        ...
        if (completeListener != null) {
            TrioConfiguration config = new TrioConfiguration(
    ...
    private void showControlConflictError() {
        String[] funnyMessages = {
                "🎮 Hé ho ! Chaque joueur humain doit avoir un contrôle différent !",
                "⚔️ Partagez les contrôles, pas les conflits !",
                "🤝 Chacun son style ! Changez de contrôle pour éviter les doublons.",
                "🎯 Même contrôle = même problème ! Variez les plaisirs !",
                "🕹️ Trois joueurs, trois contrôles différents, s'il vous plaît !",
    ...
    private void showColorConflictError() {
        String[] funnyMessages = {
                ...
                "💫 Trois équipes, trois couleurs différentes, s'il vous plaît !",
    ...
    private void selectNextField() {
        ...
        if (selectedField >= maxFields) {
            selectedField = 0;
            selectedTeam = (selectedTeam + 1) % NUM_TEAMS;
        }
        repaint();
    }
    ...
    private void selectPreviousField() {
        ...
        if (selectedField < 0) {
            selectedTeam = (selectedTeam - 1 + NUM_TEAMS) % NUM_TEAMS;
            int maxFields = getMaxFields(selectedTeam);
    ...
    protected void paintComponent(Graphics g) {
        ...
        String title = "CONFIGURATION TRIO LOCAL";
        int titleWidth = fm.stringWidth(title);
        g2d.drawString(title, (width - titleWidth) / 2, height / 10 + 20);

        // Configuration pour chaque équipe - taille ajustée pour 3 équipes
        int startY = height / 6 + 60;
        int teamSpacing = 220;
        ...
        String legendLine = "Flèches : ↑ ↓ ← →  |  Souris : Déplacer la souris  |  ZQSD : Z↑ Q← S↓ D→";

        fm = g2d.getFontMetrics();
        int lineWidth = fm.stringWidth(legendLine);
        g2d.drawString(legendLine, (width - lineWidth) / 2, legendY);
        ...
        g2d.drawString(instr, (width - instrWidth) / 2, height - 50);
    }
